...
| **Il ponte cacciato dal broker ~25 volte al giorno** | Nel log di Mosquitto: `serra-ponte disconnected: exceeded timeout`. Il WiFi non cadeva mai e il loop non poteva bloccarsi (il watchdog e' a 30 s, il broker tollera 45 s), quindi i pacchetti partivano ma non arrivavano. Era l'algoritmo di Nagle: il PINGREQ di MQTT e' lungo due byte e restava in ostaggio nel buffer di uscita in attesa che il TCP confermasse la pubblicazione precedente | `espClient.setNoDelay(true)` dopo ogni connessione, perche' il socket e' nuovo ogni volta |
...
| Il nodo si riavvia e non si sa perche' | Guarda l'entita' **`tp`**: dice in che tappa del ciclo si era fermato l'ultima volta (`tx_stato` = trasmissione LoRa, `sd`/`sd_accoda`/`backlog` = microSD, `orologio`/`sensori` = bus I2C, `irrigazione` = valvola aperta). Compare solo nel pacchetto che segue un riavvio anomalo, insieme a `fw` e `rst` |
