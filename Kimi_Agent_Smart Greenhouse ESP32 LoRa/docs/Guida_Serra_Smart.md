...
| **Contatori di sequenza azzerati a ogni riavvio** | `g_seq` stava in `RTC_DATA_ATTR`, che il bootloader riazzera a ogni avvio diverso dal risveglio dal deep sleep. Un riavvio il cui pacchetto non arrivava al ponte diventava cosi' invisibile | Spostati in `RTC_NOINIT_ATTR`, azzerati solo quando manca la corrente: ora un buco nella numerazione e' la prova di un ciclo perso |
| **Il ponte cacciato dal broker ~25 volte al giorno** | Nel log di Mosquitto: `serra-ponte disconnected: exceeded timeout`. Il WiFi non cadeva mai e il loop non poteva bloccarsi (il watchdog e' a 30 s, il broker tollera 45 s), quindi i pacchetti partivano ma non arrivavano. Nella 2.3.0 si era attribuita la colpa all'algoritmo di Nagle, che trattiene il PINGREQ da due byte | `espClient.setNoDelay(true)` dopo ogni connessione (ponte 2.3.0). **Non e' bastato**: le cadute sono rimaste 14-22 al giorno, vedi la riga seguente |
| **Dati confermati al nodo e mai arrivati** (ponte 2.4.0, nodo 2.5.0) | Le cadute sono stalli del WiFi: quasi nessuna di notte, un picco fra le 6 e le 7 e poi tutto il giorno, cioe' il profilo di un disturbo legato alla vita di casa. Le ritrasmissioni TCP, sempre piu' distanziate, fanno di pochi secondi di disturbo un silenzio di 45 s. Il ponte mandava l'ACK appena `publish()` accettava i byte, quindi ogni pacchetto del nodo che cadeva dentro uno stallo andava perso senza passare dal backlog: 12 dei 15 cicli persi dal 13/09. Inoltre un record del backlog vecchio di 15 minuti passava per fresco e sovrascriveva lo stato in HA | **Eco di conferma**: l'ACK parte solo quando il broker rimanda indietro un gettone pubblicato dopo i dati. Keepalive da 30 a 60 s, ora che non rischia piu' dati. Record del backlog marcati `bk=1` e pubblicati sempre come storici. Gli eventi (esito dell'irrigazione, diagnostica di un riavvio) si ripetono finche' non arrivano |
| **Trasmissione dell'ACK senza timeout sul ponte** | Stesso `LoRa.endPacket()` bloccante del nodo. Non si era ancora manifestato, ma il ponte e' l'unico che ascolta: mentre e' appeso non riceve da nessuno, e l'unica via d'uscita era il riavvio dopo 30 s | Guardia pari a tre volte il tempo di volo calcolato, come sul nodo |
...
| `sensor.serra_motivo_ultimo_riavvio` a **6** | Watchdog: il ciclo di veglia si e' bloccato per oltre `WDT_SETUP_SEC` (60 s dal firmware 2.4.0, prima 180). Nello storico si riconosce da un buco di circa 900 + 61 s fra due pacchetti del nodo, con `sensor.serra_ora_del_nodo` che salta in avanti. Attenzione a non contare le ripubblicazioni MQTT del ponte: valgono come riavvii solo le righe in cui l'ora dichiarata dal nodo e' nuova |
| Il nodo si riavvia e non si sa perche' | Guarda **`sensor.serra_tappa_ultimo_blocco`** (il campo `tp`): dice in che tappa del ciclo si era fermato l'ultima volta (`tx_stato` = trasmissione LoRa, `sd`/`sd_accoda`/`backlog` = microSD, `orologio`/`sensori` = bus I2C, `irrigazione` = valvola aperta). Compare dopo un riavvio anomalo, insieme a `fw` e `rst`, e si ripete finche' un pacchetto che lo contiene non arriva. Col ponte fino alla 2.3.0 l'entita' era numerica e restava sempre vuota: il testo si leggeva solo nel messaggio MQTT |
| `sensor.serra_tentativi_trasmissione` sopra 1 | Il pacchetto principale del ciclo precedente ha avuto bisogno di ritrasmissioni: 2 o 3 tentativi, 4 se non e' passato ed e' finito nel backlog. Da tenere d'occhio soprattutto nel pacchetto che segue l'irrigazione: il 22/09 e' passato al terzo e ultimo tentativo |
| `sensor.serra_conferme_mqtt_mancate` che cresce | Pacchetti del nodo rimasti senza ACK perche' il broker non ha rimandato l'eco in tempo: il WiFi del ponte era in stallo. Non e' una perdita: il nodo li ritrasmette o li tiene sulla microSD, mentre la 2.3.0 li confermava al buio. Se cresce di molte unita' al giorno, conviene avvicinare il ponte all'access point |
| `sensor.serra_record_in_backlog` a **-1** | La microSD del nodo e' assente o guasta: i pacchetti non consegnati non hanno dove andare e si perdono. In HA lo stato generale dice "MicroSD guasta" |
| Entita' che lampeggiano, e si vuole sapere di chi e' la colpa | Nella diagnostica del ponte c'e' **`mqtt_caduta`**, cioe' `mqtt.state()` letto nell'istante della caduta: **-4** vuol dire che e' stato il ponte a chiudere perche' il suo ping non ha avuto risposta, **-3** che se l'e' vista chiudere sotto dal broker. **`rssi_caduta`** dice com'era il segnale WiFi in quel momento: se e' molto peggiore di `wifi_rssi`, il problema e' la copertura e non il software |
